...
`;function $e(e,t={}){let{paused:o=false}=t,r=react.useRef(null),i=react.useRef(null),c=react.useRef(null),a=react.useRef(null),f=react.useRef(o),b=react.useRef(null),u=react.useMemo(()=>({color1:ae(e.color1),color2:ae(e.color2),color3:ae(e.color3)}),[e.color1,e.color2,e.color3]);return react.useEffect(()=>{var Q;let m=r.current;if(!m)return;let g=new ogl.Renderer({canvas:m,dpr:Math.min(window.devicePixelRatio,2),alpha:false,antialias:false,powerPreference:"high-performance"});i.current=g;let P=g.gl,A=new ogl.Plane(P,{width:2,height:2}),O=()=>{var H;if(!m.parentElement)return;let _=m.parentElement,C=_.clientWidth,v=_.clientHeight,E=Math.min(window.devicePixelRatio,2);m.width=C*E,m.height=v*E,m.style.width=C+"px",m.style.height=v+"px",g.setSize(C,v),c.current&&(c.current.uniforms.u_resolution.value=[C,v]),(H=b.current)==null||H.renderFrame();},k=new ogl.Program(P,{vertex:Xo,fragment:Jo,uniforms:{u_time:{value:0},u_color1:{value:u.color1},u_color2:{value:u.color2},u_color3:{value:u.color3},u_speed:{value:e.speed},u_scale:{value:e.scale},u_type:{value:se[(Q=e.type)!=null?Q:"animated"]},u_noise:{value:e.noise},u_resolution:{value:[m.clientWidth,m.clientHeight]}}});c.current=k;let R=new ogl.Mesh(P,{geometry:A,program:k});a.current=R;let F=new ogl.Transform;R.setParent(F),O(),window.addEventListener("resize",O,{passive:true});let x=window.matchMedia("(prefers-reduced-motion: reduce)"),S={inView:true,pageVisible:!document.hidden,reducedMotion:x.matches},z=0,N=0,I=null,M=()=>g.render({scene:R}),l=_=>{I!==null&&(N+=(_-I)/1e3),I=_,k.uniforms.u_time.value=N,M(),z=requestAnimationFrame(l);},h=()=>{let _=!f.current&&!S.reducedMotion&&S.inView&&S.pageVisible;_&&!z?(I=null,z=requestAnimationFrame(l)):!_&&z&&(cancelAnimationFrame(z),z=0);};b.current={sync:h,renderFrame:M};let q=new IntersectionObserver(([_])=>{S.inView=_.isIntersecting,h();});q.observe(m);let $=()=>{S.pageVisible=!document.hidden,h();};document.addEventListener("visibilitychange",$);let V=()=>{S.reducedMotion=x.matches,h();};return x.addEventListener("change",V),M(),h(),()=>{var C,v;cancelAnimationFrame(z),q.disconnect(),document.removeEventListener("visibilitychange",$),x.removeEventListener("change",V),b.current=null,window.removeEventListener("resize",O);let _=(C=i.current)==null?void 0:C.gl;(v=c.current)!=null&&v.program&&_&&_.deleteProgram(c.current.program),i.current=null,c.current=null,a.current=null;}},[]),react.useEffect(()=>{var m;f.current=o,(m=b.current)==null||m.sync();},[o]),react.useEffect(()=>{var g,P;let m=c.current;m&&(m.uniforms.u_color1.value=u.color1,m.uniforms.u_color2.value=u.color2,m.uniforms.u_color3.value=u.color3,m.uniforms.u_speed.value=e.speed,m.uniforms.u_scale.value=e.scale,m.uniforms.u_type.value=se[(g=e.type)!=null?g:"animated"],m.uniforms.u_noise.value=e.noise,(P=b.current)==null||P.renderFrame());},[e,u]),{canvasRef:r,rendererRef:i,programRef:c,meshRef:a}}function Qe(e){let t=e.replace("#",""),o=parseInt(t,16),r=o>>16&255,i=o>>8&255,c=o&255;return {r,g:i,b:c}}function Ko(e){let{r:t,g:o,b:r}=e;return "#"+((1<<24)+(t<<16)+(o<<8)+r).toString(16).slice(1)}function K(e){return typeof e=="string"?Qe(e):e}function he({config:e,preset:t,paused:o=false,className:r=""}){let i=react.useMemo(()=>{let a=X(X({},fe),t?be[t]:void 0);return e&&(e.color1&&(a.color1=K(e.color1)),e.color2&&(a.color2=K(e.color2)),e.color3&&(a.color3=K(e.color3)),e.speed!==void 0&&(a.speed=e.speed),e.scale!==void 0&&(a.scale=e.scale),e.type&&(a.type=e.type),e.noise!==void 0&&(a.noise=e.noise)),a},[e,t]),{canvasRef:c}=$e(i,{paused:o});return jsxRuntime.jsx("canvas",{ref:c,className:qe("w-full h-full block select-none touch-none",r),"aria-label":"gradflow animated gradient background"})}function ie(){return {r:Math.floor(Math.random()*256),g:Math.floor(Math.random()*256),b:Math.floor(Math.random()*256)}}function or(){return {color1:ie(),color2:ie(),color3:ie()}}exports.DEFAULT_CONFIG=fe;exports.GRADIENT_TYPE_NUMBER=se;exports.GradFlow=he;exports.PRESETS=be;exports.default=he;exports.generateRandomColors=or;exports.hexToRgb=Qe;exports.normalizeColor=K;exports.randomRGB=ie;exports.rgbToHex=Ko;