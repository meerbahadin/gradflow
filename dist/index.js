...
`;function Ve(e,t={}){let{paused:o=false}=t,r=Eo.useRef(null),i=Eo.useRef(null),c=Eo.useRef(null),a=Eo.useRef(null),f=Eo.useRef(o),u=Eo.useRef(null),g=Eo.useMemo(()=>({color1:se(e.color1),color2:se(e.color2),color3:se(e.color3)}),[e.color1,e.color2,e.color3]);return Eo.useEffect(()=>{var Y;let m=r.current;if(!m)return;let x=new ogl.Renderer({canvas:m,dpr:Math.min(window.devicePixelRatio,2),alpha:false,antialias:false,powerPreference:"high-performance"});i.current=x;let T=x.gl,U=new ogl.Plane(T,{width:2,height:2}),w=()=>{var l;if(!m.parentElement)return;let h=m.parentElement,M=h.clientWidth,y=h.clientHeight,I=Math.min(window.devicePixelRatio,2);m.width=M*I,m.height=y*I,m.style.width=M+"px",m.style.height=y+"px",x.setSize(M,y),c.current&&(c.current.uniforms.u_resolution.value=[M,y]),(l=u.current)==null||l.renderFrame();},k=new ogl.Program(T,{vertex:zo,fragment:Io,uniforms:{u_time:{value:0},u_color1:{value:g.color1},u_color2:{value:g.color2},u_color3:{value:g.color3},u_speed:{value:e.speed},u_scale:{value:e.scale},u_type:{value:ne[(Y=e.type)!=null?Y:"animated"]},u_noise:{value:e.noise},u_resolution:{value:[m.clientWidth,m.clientHeight]}}});c.current=k;let z=new ogl.Mesh(T,{geometry:U,program:k});a.current=z;let C=new ogl.Transform;z.setParent(C),w(),window.addEventListener("resize",w,{passive:true});let E=window.matchMedia("(prefers-reduced-motion: reduce)"),A={inView:true,pageVisible:!document.hidden,reducedMotion:E.matches},G=0,F=0,S=null,L=()=>x.render({scene:z}),d=h=>{S!==null&&(F+=(h-S)/1e3),S=h,k.uniforms.u_time.value=F,L(),G=requestAnimationFrame(d);},_=()=>{let h=!f.current&&!A.reducedMotion&&A.inView&&A.pageVisible;h&&!G?(S=null,G=requestAnimationFrame(d)):!h&&G&&(cancelAnimationFrame(G),G=0);};u.current={sync:_,renderFrame:L};let q=new IntersectionObserver(([h])=>{A.inView=h.isIntersecting,_();});q.observe(m);let $=()=>{A.pageVisible=!document.hidden,_();};document.addEventListener("visibilitychange",$);let B=()=>{A.reducedMotion=E.matches,_();};return E.addEventListener("change",B),L(),_(),()=>{var M,y;cancelAnimationFrame(G),q.disconnect(),document.removeEventListener("visibilitychange",$),E.removeEventListener("change",B),u.current=null,window.removeEventListener("resize",w);let h=(M=i.current)==null?void 0:M.gl;(y=c.current)!=null&&y.program&&h&&h.deleteProgram(c.current.program),i.current=null,c.current=null,a.current=null;}},[]),Eo.useEffect(()=>{var m;f.current=o,(m=u.current)==null||m.sync();},[o]),Eo.useEffect(()=>{var x,T;let m=c.current;m&&(m.uniforms.u_color1.value=g.color1,m.uniforms.u_color2.value=g.color2,m.uniforms.u_color3.value=g.color3,m.uniforms.u_speed.value=e.speed,m.uniforms.u_scale.value=e.scale,m.uniforms.u_type.value=ne[(x=e.type)!=null?x:"animated"],m.uniforms.u_noise.value=e.noise,(T=u.current)==null||T.renderFrame());},[e,g]),{canvasRef:r,rendererRef:i,programRef:c,meshRef:a}}function We(e){let t=e.replace("#",""),o=parseInt(t,16),r=o>>16&255,i=o>>8&255,c=o&255;return {r,g:i,b:c}}function Co(e){let{r:t,g:o,b:r}=e;return "#"+((1<<24)+(t<<16)+(o<<8)+r).toString(16).slice(1)}function X(e){return typeof e=="string"?We(e):e}function ge({config:e,preset:t,paused:o=false,className:r=""}){let i=Eo.useMemo(()=>{let a=J(J({},me),t?pe[t]:void 0);return e&&(e.color1&&(a.color1=X(e.color1)),e.color2&&(a.color2=X(e.color2)),e.color3&&(a.color3=X(e.color3)),e.speed!==void 0&&(a.speed=e.speed),e.scale!==void 0&&(a.scale=e.scale),e.type&&(a.type=e.type),e.noise!==void 0&&(a.noise=e.noise)),a},[e,t]),{canvasRef:c}=Ve(i,{paused:o});return Eo__default.default.createElement("canvas",{ref:c,className:Be("w-full h-full block select-none touch-none",r),"aria-label":"gradflow animated gradient background"})}function ae(){return {r:Math.floor(Math.random()*256),g:Math.floor(Math.random()*256),b:Math.floor(Math.random()*256)}}function Lo(){return {color1:ae(),color2:ae(),color3:ae()}}exports.DEFAULT_CONFIG=me;exports.GRADIENT_TYPE_NUMBER=ne;exports.GradFlow=ge;exports.PRESETS=pe;exports.default=ge;exports.generateRandomColors=Lo;exports.hexToRgb=We;exports.normalizeColor=X;exports.randomRGB=ae;exports.rgbToHex=Co;